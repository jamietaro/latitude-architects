...
/**
 * Shared entry editor used by both the Journal and News admin sections.
 * Each section passes its own `apiPath`, so the two are entirely separate
 * flows that manage only their own model.
 */
export default function EntryManager({
  ...
  function selectPost(post: Entry) {
    setSelected(post);
    setIsNew(false);
    setForm({
      title: post.title,
  ...
  // Always include the current value so an existing category outside the
  // configured list still displays correctly.
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-5">
            ...
            <div className="pt-5 border-t border-[#2a2a2e]">
              ...
              <p className="text-[#666] text-xs mb-2">
                Additional images shown stacked below the {noun} body.
              </p>
              <GalleryUpload
                images={form.images}
